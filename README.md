...
like a fresh install, and wryayer takes it at face value: installs land
*underneath* the mount point, and a prompt announces that no master password
store exists and offers to make one. Mounting the container then hides that
second copy, and the next boot brings it back, so the master password you know
is rejected by a store you never created.

Mount the container before you run wryayer.

If that has already happened, there may be a shadow copy underneath the mount
point. A non-recursive bind mount shows what is hidden
