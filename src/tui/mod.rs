...
/// Make one line of a child's output safe to draw inside the TUI.
///
/// Log lines end up in a ratatui `Paragraph`, which passes their bytes through
/// to the terminal untouched. Anything the child emitted for a *terminal* to act
/// on therefore acts on the TUI's own screen instead, and the layout comes
/// apart. Two things arrive routinely:
///
...
///
/// Deliberately done here, where output enters the log, rather than at draw
/// time: the stored log is then clean for everything that reads it.
    ...
    thread::spawn(move || {
        ...
        thread::spawn(move || {
            for line in BufReader::new(stderr).lines().map_while(Result::ok) {
                let _ = tx2.send(Msg::Line(sanitize_log_line(&line)));
            }
        });

        if let Some(stdout) = child.stdout.take() {
            for line in BufReader::new(stdout).lines().map_while(Result::ok) {
                let _ = tx.send(Msg::Line(sanitize_log_line(&line)));
            }
        }
...
mod op_log_tests {
    ...
    fn colour_escapes_do_not_outlive_their_line() {
        ...
        let out = sanitize_log_line("\u{1b}[32mgreen\u{1b}[0m and \u{1b}[1;31mred");
        assert_eq!(out, "green and red");
    }

    #[test]
    fn stray_control_bytes_are_dropped() {
        let out = sanitize_log_line("bell\u{7}nul\u{0}del\u{7f}done");
        assert_eq!(out, "bellnuldeldone");
    }

    #[test]
    fn tabs_become_spaces_rather_than_terminal_tab_stops() {
        assert_eq!(sanitize_log_line("a\tb"), "a    b");
    }
    ...
    fn a_line_of_pure_control_bytes_becomes_empty_not_garbage() {
        assert_eq!(sanitize_log_line("\u{1b}[2J\u{1b}[H"), "");
    }
