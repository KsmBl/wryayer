...
pub fn setting_description(idx: usize) -> &'static str {
    match idx {
        ...
        CFG_GPU => "Which GPU this app renders on.\n\nSets DRI_PRIME and MESA_VK_DEVICE_SELECT for Mesa, and the __NV_PRIME/__GLX_VENDOR_LIBRARY_NAME/__VK_LAYER_NV_optimus trio when an NVIDIA card is involved — each driver stack reads its own.\n\nThe render nodes of the GPUs the app may not use are then hidden from the sandbox, so an app that enumerates devices itself cannot pick one anyway. The card driving your screen is never hidden: the app still allocates and shares buffers through it, and taking it away drops the app to software rendering instead of moving it.\n\n• auto — let the driver choose (the integrated GPU, on a hybrid machine)\n• <card> — pin the app to that GPU\n\nOn a machine with one GPU this changes nothing.",
